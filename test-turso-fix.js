...
// Wrapper to trick Sequelize
const { Database } = sqlite3;
class CustomDatabase extends Database {
  constructor(filename, ...args) {
    // If filename is :memory: and we have a dbUrl, use dbUrl
    if (filename === ':memory:' && dbUrl) {
      console.log("Redirecting :memory: to Turso URL");
      super(dbUrl, ...args);
    } else {
      super(filename, ...args);
    }
...
const sequelize = new Sequelize({
  ...
  storage: ":memory:", // Trick Sequelize to skip mkdir
  logging: console.log,
});
